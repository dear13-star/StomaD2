import os
from typing import overload, Generator, Dict, List
...
from .common import instantiate_from_config, load_file_from_url, count_vram_usage
...
class InferenceLoop:
    ...
    def init_stage2_model(self) -> None:
        ### load uent, vae, clip
        self.cldm: ControlLDM = instantiate_from_config(OmegaConf.load("DiffBIR/configs/inference/cldm.yaml"))
        ...
        ### load diffusion
        self.diffusion: Diffusion = instantiate_from_config(OmegaConf.load("DiffBIR/configs/inference/diffusion.yaml"))
...
class BSRInferenceLoop(InferenceLoop):
    ...
    @count_vram_usage
    def init_stage1_model(self) -> None:
        self.bsrnet: RRDBNet = instantiate_from_config(OmegaConf.load("DiffBIR/configs/inference/bsrnet.yaml"))
...
class BFRInferenceLoop(InferenceLoop):
    ...
    @count_vram_usage
    def init_stage1_model(self) -> None:
        self.swinir_face: SwinIR = instantiate_from_config(OmegaConf.load("DiffBIR/configs/inference/swinir.yaml"))
...
class BIDInferenceLoop(InferenceLoop):
    ...
    @count_vram_usage
    def init_stage1_model(self) -> None:
        self.scunet_psnr: SCUNet = instantiate_from_config(OmegaConf.load("DiffBIR/configs/inference/scunet.yaml"))
...
class V1InferenceLoop(InferenceLoop):
    ...
    @count_vram_usage
    def init_stage1_model(self) -> None:
        self.swinir: SwinIR = instantiate_from_config(OmegaConf.load("DiffBIR/configs/inference/swinir.yaml"))
        if self.args.task == "fr":
            sd = load_model_from_url(MODELS["swinir_face"])
        elif self.args.task == "sr":
